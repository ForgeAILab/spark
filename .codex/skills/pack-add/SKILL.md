---
name: pack-add
description: Safely install feature packs by dry-running `app-skills add`, showing the plan, waiting for explicit approval, running the real install, and syncing the board. Use when the user asks to add one or more packs.
...
- Always run `app-skills add <pack...> --dry-run` before any real install.
...
- On approval, run `app-skills add <pack...>` with the same pack arguments as the dry-run.
...
2. Run `app-skills add <pack...> --dry-run`.
3. Summarize the plan and ask the user for explicit approval.
4. If approved, run `app-skills add <pack...>`.
