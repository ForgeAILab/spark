...
export async function readConfig(projectRoot: string): Promise<AppSkillsConfig> {
  const configPath = join(projectRoot, 'anvil.config.json');
  let raw: string;
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new Error('not in an anvil project: missing anvil.config.json');
    }
    throw error;
  }
  ...
  if (!isRecord(parsed) || typeof parsed.template !== 'string' || parsed.template.length === 0) {
    throw new Error('anvil.config.json must include a non-empty "template" field');
  }
