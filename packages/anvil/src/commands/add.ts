...
import type { StateInstalledPack } from '@forgeailab/anvil-schema';
...
function activeTemplateFromRegistry(config: AppSkillsConfig, registry: Registry): ResolverTemplate {
  ...
  if (!template) {
    throw new Error(`Template "${config.template}" is not registered in the anvil template registry.`);
  }
...
export const addCommand = defineCommand({
  meta: {
    name: 'add',
    description: 'Install one or more anvil packs',
  },
  args: {
    dryRun: {
      type: 'boolean',
