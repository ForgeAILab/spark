...
const mainCommand = defineCommand({
  meta: {
    name: 'anvil',
    description: 'Composable anvil scaffold pack manager',
  },
  subCommands,
});
...
function guardUnknownSubcommand(argv: readonly string[]): void {
  ...
  if (command === 'remove' || command === 'uninstall' || command === 'update') {
    console.error(
      `unknown subcommand "${command}". anvil does not support uninstall/update in v1; use git revert to undo a pack install.`,
    );
    process.exit(1);
  }
