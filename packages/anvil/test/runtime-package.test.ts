...
import type { PackManifest, TemplateManifest } from '@forgeailab/anvil-schema';
...
const helperPackage = '@forgeailab/anvil-auth-better-auth';
...
async function setupHybridFixture(packageJson = '{}\n') {
  const root = await mkdtemp(join(tmpdir(), 'anvil-runtime-package-'));
  ...
  await writeFile(join(projectRoot, 'anvil.config.json'), '{"template":"nextjs","appName":"demo"}\n');
...
describe('runtime packages', () => {
  test('published mode adds the helper package version range to the runtime bun add batch', async () => {
    const originalAnvilRoot = process.env.ANVIL_ROOT;
    ...
    try {
      ...
      if (originalAnvilRoot === undefined) {
        delete process.env.ANVIL_ROOT;
      } else {
        process.env.ANVIL_ROOT = originalAnvilRoot;
      }
    }
  });

  test('dev mode adds a file link when ANVIL_ROOT points at a local helper package', async () => {
    const originalAnvilRoot = process.env.ANVIL_ROOT;

    try {
      const { root, projectRoot, registry } = await setupHybridFixture();
      const monorepoRoot = join(root, 'monorepo');
      const helperDir = join(monorepoRoot, 'libs', 'anvil-auth-better-auth');
      await mkdir(helperDir, { recursive: true });
      await writeFile(join(helperDir, 'package.json'), '{"name":"@forgeailab/anvil-auth-better-auth"}\n');
      process.env.ANVIL_ROOT = monorepoRoot;
  ...
  test('info prints hybrid mode and the resolved helper specifier from package.json', async () => {
    ...
    expect(output).toContain(
      'Runtime helper: @forgeailab/anvil-auth-better-auth (range ^0.1, resolved 0.1.4)',
    );
  });
  ...
  test('check reports drift when an installed hybrid pack helper is absent from package.json', async () => {
    ...
    expect(report.missingHelpers).toEqual([
      'auth-better-auth: helper package @forgeailab/anvil-auth-better-auth missing from package.json',
    ]);
