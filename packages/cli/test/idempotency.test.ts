...
import type { PackManifest, TemplateManifest } from '@app-skills/pack-schema';
...
async function setupProject() {
  const root = await mkdtemp(join(tmpdir(), 'app-skills-idempotency-'));
  ...
  await writeFile(join(projectRoot, 'app-skills.config.json'), '{"template":"nextjs","appName":"demo"}\n');
...
describe('add idempotency', () => {
  test('full add followed by same add leaves the project unchanged', async () => {
    ...
    expect(afterSecond['.app-skills/state.json']).toContain('"demo-pack"');
  });
});
