...
import type { StateInstalledPack } from '@app-skills/pack-schema';
...
function activeTemplateFromRegistry(config: AppSkillsConfig, registry: Registry): ResolverTemplate {
  ...
  if (!template) {
    throw new Error(`Template "${config.template}" is not registered in the app-skills template registry.`);
  }
...
export const addCommand = defineCommand({
  meta: {
    name: 'add',
    description: 'Install one or more app-skills packs',
  },
  args: {
    dryRun: {
      type: 'boolean',
