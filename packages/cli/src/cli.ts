...
const mainCommand = defineCommand({
  meta: {
    name: 'app-skills',
    description: 'Composable app-skills scaffold pack manager',
  },
  subCommands,
});
...
function guardUnknownSubcommand(argv: readonly string[]): void {
  ...
  if (command === 'remove' || command === 'uninstall' || command === 'update') {
    console.error(
      `unknown subcommand "${command}". app-skills does not support uninstall/update in v1; use git revert to undo a pack install.`,
    );
    process.exit(1);
  }
