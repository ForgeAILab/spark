...
import { parseTemplateToml, type ParseError, type TemplateManifest } from '@forgeailab/anvil-schema';
import { findMonorepoRoot } from './paths.ts';

export type TemplateMetadata = TemplateManifest;

export function getTemplatesDir(): string {
  return process.env.CREATE_ANVIL_TEMPLATES_DIR ?? join(findMonorepoRoot(), 'templates');
}
