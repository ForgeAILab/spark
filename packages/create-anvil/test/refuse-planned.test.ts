...
async function createTemplateRegistry(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'create-anvil-planned-'));
  const manifests = [
    {
      name: 'nextjs',
      status: 'stable',
...
async function runCli(templateName: string, templatesDir: string): Promise<{
  ...
  const workspace = await mkdtemp(join(tmpdir(), 'create-anvil-run-'));
  ...
  try {
    const proc = Bun.spawn(
      ['bun', '--bun', cliPath, 'my-app', '--template', templateName, '--yes'],
      {
        ...
        env: {
          ...process.env,
          CREATE_ANVIL_TEMPLATES_DIR: templatesDir,
          NO_COLOR: '1',
        },
        stdout: 'pipe',
        stderr: 'pipe',
