...
import { parseTemplateToml, type ParseError, type TemplateManifest } from '@app-skills/pack-schema';
import { findMonorepoRoot } from './paths.ts';

export type TemplateMetadata = TemplateManifest;

export function getTemplatesDir(): string {
  return process.env.CREATE_APP_SKILLS_TEMPLATES_DIR ?? join(findMonorepoRoot(), 'templates');
}
