...
Every pack uses the copy model: its `[[files]]` entries are copied into the
consumer project, and the user owns the resulting code in place. This is the
...
The spark repo includes these top-level directories:
