...
- **WHEN** a user with Bun ≥ 1.3 installed runs `bunx app-skills list` in a scaffolded project
...
- **WHEN** a user with no Rust toolchain installed runs any `app-skills` subcommand
...
- **WHEN** the user runs `app-skills list` in a scaffolded project with `db-sqlite` and `ui-shadcn` installed
...
- **WHEN** the user runs `app-skills add payments-stripe --dry-run`
...
- **AND** `.app-skills/state.json` is unchanged

#### Scenario: `remove` subcommand does not exist

- **WHEN** the user runs `app-skills remove db-sqlite`
...
`app-skills add <pack>` MUST be idempotent. Running it twice with the same arguments MUST produce the same end state as running it once and MUST NOT duplicate file writes, env-var appends, or seeded board tasks.

#### Scenario: Second add is a no-op

- **WHEN** the user runs `app-skills add db-sqlite` and the install succeeds
- **AND** the user immediately runs `app-skills add db-sqlite` again
...
- **WHEN** the user runs `app-skills add payments-stripe` in a project with no `auth` capability installed
...
- **WHEN** the user installs `payments-stripe` and then attempts `app-skills add payments-polar`
...
- **WHEN** the active project's `app-skills.config.json` declares `template = "nextjs"`
...
- **THEN** `app-skills check` exits 0 with an "OK" summary
...
- **THEN** `app-skills check` exits non-zero
...
- **THEN** `app-skills check` reports the missing file under a "drift" section
