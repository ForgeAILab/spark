...
The skill SHALL prompt the author to choose an install mode: `copy` or `hybrid`. When the author selects `hybrid`, the skill additionally scaffolds `libs/spark-<name>/` with a minimal workspace package skeleton (`package.json`, `tsconfig.json`, `src/index.ts`, `test/index.test.ts`, `README.md`) AND writes a `[runtime_package]` block into the new `pack.toml` referencing `@forgeailab/spark-<name>` with an initial version range of `"^0.1"`.
...
- **AND** does NOT create `libs/spark-realtime-supabase/`
...
- **AND** the new `pack.toml` includes a `[runtime_package]` table with `package = "@forgeailab/spark-realtime-supabase"` and `version = "^0.1"`
- **AND** creates `libs/spark-realtime-supabase/` with a minimal workspace package skeleton
- **AND** the new workspace package declares `name = "@forgeailab/spark-realtime-supabase"` and `version = "0.1.0"`
...
- **WHEN** `auth-better-auth` is installed with `@forgeailab/spark-auth-better-auth@0.1.0` in the consumer's `package.json`
- **AND** `@forgeailab/spark-auth-better-auth@0.3.5` is the latest version on the npm registry
- **THEN** the risk-check output contains a "Stale helper" section
- **AND** lists `auth-better-auth` with the suggested command `bun update @forgeailab/spark-auth-better-auth`

#### Scenario: Helper version within tolerance produces no false positive

- **WHEN** `auth-better-auth` is installed with `@forgeailab/spark-auth-better-auth@0.2.0`
