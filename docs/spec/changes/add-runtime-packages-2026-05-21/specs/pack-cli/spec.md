...
- **WHEN** the user runs `spark add auth-better-auth`
- **AND** `packs/auth-better-auth/pack.toml` declares `[runtime_package] package = "@forgeailab/spark-auth-better-auth"`, `version = "^0.1"`
- **THEN** the CLI invokes `bun add @forgeailab/spark-auth-better-auth@^0.1` as part of the install
- **AND** the consumer's `package.json` lists `@forgeailab/spark-auth-better-auth` under dependencies
...
- **WHEN** the user runs `spark add db-sqlite`
...
- **WHEN** the user runs `spark add auth-better-auth` against a project whose `spark.config.json` sits outside the monorepo
- **AND** `SPARK_ROOT` is set to the monorepo root
- **AND** `libs/spark-auth-better-auth/package.json` exists at `${SPARK_ROOT}/libs/spark-auth-better-auth/`
- **THEN** the CLI invokes `bun add file:${SPARK_ROOT}/libs/spark-auth-better-auth` (or an equivalent relative `file:` path)
- **AND** the consumer's `package.json` lists `@forgeailab/spark-auth-better-auth` with a `file:` dep specifier
...
- **WHEN** the user runs `spark add auth-better-auth` against a project where neither `SPARK_ROOT` is set nor a local `libs/spark-auth-better-auth/` is reachable from the project's spark monorepo lookup
- **AND** `packs/auth-better-auth/pack.toml` declares `[runtime_package].version = "^0.1"`
- **THEN** the CLI invokes `bun add @forgeailab/spark-auth-better-auth@^0.1`
...
For every pack recorded in `.spark/state.json` whose manifest declares `[runtime_package]`, `spark check` SHALL verify that the helper package is still listed in the consumer project's `package.json` (as a `dependencies` or `devDependencies` entry; `dependencies` is the expected location). When the helper is missing, `check` MUST report drift with a clear "helper package missing" line naming the pack and the helper package.

#### Scenario: Missing hybrid helper is surfaced as drift

- **WHEN** the user has installed `auth-better-auth`
- **AND** the user manually removes `@forgeailab/spark-auth-better-auth` from `package.json`
- **AND** the user runs `spark check`
- **THEN** the exit code is non-zero
- **AND** the output lists `auth-better-auth: helper package @forgeailab/spark-auth-better-auth missing from package.json` under a drift section
- **AND** `check` does NOT repair the drift

#### Scenario: Helper present produces no false positive

- **WHEN** the user has installed `auth-better-auth`
- **AND** `@forgeailab/spark-auth-better-auth` is present in `package.json`
- **THEN** `spark check` does not flag the pack under any helper-related drift section
